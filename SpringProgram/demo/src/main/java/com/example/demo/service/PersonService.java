...
public class PersonService implements UserDetailsService{
    ...
    public String ApplicantUseResetLink(String resetToken,String username, String password)
    {
        try {            
            ...
            Optional<ApplicantReset> resetContainer=applicantResetRepository.findByPersonAndResetDateAndRandomLong(person,jwtService.extractExpiration(resetToken).toString(),jwtService.extractRandomNumber(resetToken));

            if (resetContainer.isEmpty()) {
                LOGGER.error("Failed to update applicant following link with token (`{}`) which contains email (`{}`) since no reset request for that person was found",resetToken,email);
                throw new InvalidJWTException("You gave an invalid but potentially real token, no current request for that person exists in the system. The link may have already been used, in which case you must request a new one.");
            }
            ...
            //We then remove the token, since we want to avoid a token being re-used in this case
